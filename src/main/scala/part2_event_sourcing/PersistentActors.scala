...
object PersistentActors extends App {
  ...
  class Accountant extends PersistentActor with ActorLogging{
    ...
    override def receiveCommand: Receive = {
      case Invoice(recipient, date, amount) =>
        /*
          ...
          2) you persist the event, the pass in a callback that will triggered once the event is written
        ...
        persist(event)/* time gap happen here: all other messages sent to this actor are STASHED*/
        { e =>
          // SAFE to access mutable state here
          // akka persistence guarantee that no others threads no accessing the actor during a callback
      ...
      case InvoiceBulk(invoices) =>
        ...
        val events = invoices.zip(invoiceIds).map { pair => // tuple with Invoice and invoiceId
            val id = pair._2
          val invoice = pair._1
    ...
    /**
      * Handler that will be called on recivery
      * @return
      */
  ...
  /**
    ...
    *
    * Best ´ractice: define your own "shutdown" messages
    */
