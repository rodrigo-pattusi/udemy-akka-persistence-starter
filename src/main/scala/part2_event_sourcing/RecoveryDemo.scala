    ...
    def online(latestPersistedEventId: Int): Receive = {
      case Command(contents) =>
        persist(Event(latestPersistedEventId, contents)) {event =>
          log.info(s"Successfiully persisted $event, recovery is ${if (this.recoveryFinished)"" else "NOT"} finished.")
...
//  }
  ...
  /*
    4 - recovery status or KNOWING when you're done recovering
      - getting a signal when you're recovering
   */

  /*
    5 - stateless actors
   */
  recoveryActor ! Command(s"special command 3")
  recoveryActor ! Command(s"special command 4")
}
