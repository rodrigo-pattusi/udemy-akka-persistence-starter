...
object MultiplePersists extends App{
  ...
  class DiligentAccountant(taxId: String, taxAuthoruty: ActorRef) extends PersistentActor with ActorLogging{
    ...
    override def receiveCommand: Receive = {
      case Invoice(recipient, date, amount) =>
        // persist it's like -> journal ! TaxRecord
        ...
        persist(InvoiceRecord(latestInvoiceRecordId, recipient, date, amount)){ invoiceRecord =>
          ...
          persist("I hereby declare this invoice to be true and complete"){ declaration =>
            taxAuthority ! declaration
          }
        }
  ...
  val accountant = system.actorOf(DiligentAccountant.props("UK52352_583253", taxAuthority))
